...
public class BT  implements GlobalConst{
  ...
  protected final static 
	...
	if ( nodeType==NodeType.INDEX ) {
	  n=4;
	  data= new IndexData( ConvertMap.getIntValue(offset+length-4, from));
	}
	else if ( nodeType==NodeType.LEAF) {
	  n=8;
	  MID rid=new MID();
	  rid.slotNo =   ConvertMap.getIntValue(offset+length-8, from);
	  rid.pageNo =new PageId();
	  rid.pageNo.pid= ConvertMap.getIntValue(offset+length-4, from); 
	...
	if ( keyType== AttrType.attrInteger) {
	  key= new IntegerKey( new Integer 
			       (ConvertMap.getIntValue(offset, from)));
	}
	else if (keyType== AttrType.attrString) {
	  //System.out.println(" offset  "+ offset + "  " + length + "  "+n);
          key= new StringKey( ConvertMap.getStrLabel(offset, from, length-n));
	} 
	else 
  ...
  protected final static byte[] getBytesFromEntry( KeyDataEntry entry ) 
	   ...
	   ConvertException
    {
	  ...
	  ConvertMap.setIntValue( ((IntegerKey)entry.key).getKey().intValue(),
			       0, data);
        }
        else if ( entry.key instanceof StringKey ) {
	  ConvertMap.setStrValue( ((StringKey)entry.key).getKey(),
			       0, data);            
        }
        else throw new KeyNotMatchException(null, "key types do not match");
        
        if ( entry.data instanceof IndexData ) {
	  ConvertMap.setIntValue( ((IndexData)entry.data).getData().pid,
			       m, data);
        }
        else if ( entry.data instanceof LeafData ) {
	  ConvertMap.setIntValue( ((LeafData)entry.data).getData().slotNo,
			       m, data);
	  ConvertMap.setIntValue( ((LeafData)entry.data).getData().pageNo.pid,
			       m+4, data);
