  ...
  */
  ...
  {           
    ...
    if ( (fldNo > 0) && (fldNo <= fldCnt))
     {
      val = ConvertMap.getIntValue(fldOffset[fldNo -1], data);
      return val;
     }
    else 
    ...
    */
	...
	float val;
      if ( (fldNo > 0) && (fldNo <= fldCnt))
       {
        val = ConvertMap.getFloValue(fldOffset[fldNo -1], data);
        return val;
       }
      else 
       throw new FieldNumberOutOfBoundException (null, "TUPLE:TUPLE_FLDNO_OUT_OF_BOUND");
   ...
   { 
    ...
    if ( (fldNo > 0) && (fldNo <= fldCnt))      
     {
        val = ConvertMap.getStrLabel(fldOffset[fldNo -1], data, 
   ...
   public char getCharFld(int fldNo) 
    ...
    {   
      ...
      if ( (fldNo > 0) && (fldNo <= fldCnt))      
       {
        val = ConvertMap.getCharValue(fldOffset[fldNo -1], data);
        return val;
       }
      else 
       throw new FieldNumberOutOfBoundException (null, "TUPLE:TUPLE_FLDNO_OUT_OF_BOUND");
  ...
  { 
    if ( (fldNo > 0) && (fldNo <= fldCnt))
     {
	ConvertMap.setIntValue (val, fldOffset[fldNo -1], data);
	return this;
     }
    else 
     throw new FieldNumberOutOfBoundException (null, "TUPLE:TUPLE_FLDNO_OUT_OF_BOUND"); 
  ...
  { 
   if ( (fldNo > 0) && (fldNo <= fldCnt))
    {
     ConvertMap.setFloValue (val, fldOffset[fldNo -1], data);
     return this;
    }
    else  
     throw new FieldNumberOutOfBoundException (null, "TUPLE:TUPLE_FLDNO_OUT_OF_BOUND"); 
 ...
 throws IOException, InvalidTypeException, InvalidTupleSizeException		
{
  ...
  fldCnt = numFlds;
  ConvertMap.setShortValue(numFlds, tuple_offset, data);
  ...
  ConvertMap.setShortValue(fldOffset[i], pos, data);
  pos +=2;
 ...
 switch(types[numFlds -1].attrType) {
  ...
  ConvertMap.setShortValue(fldOffset[numFlds], pos, data);
    ...
    throws IOException 
 {
  ...
  for (i=0; i< fldCnt-1; i++)
   ...
   case AttrType.attrInteger:
     val = ConvertMap.getIntValue(fldOffset[i], data);
     System.out.print(val);
     break;

   case AttrType.attrReal:
     fval = ConvertMap.getFloValue(fldOffset[i], data);
     System.out.print(fval);
     break;

   case AttrType.attrString:
     sval = ConvertMap.getStrLabel(fldOffset[i], data,fldOffset[i+1] - fldOffset[i]);
